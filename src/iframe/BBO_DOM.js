...
function setOptions(on) {
	...
	if (on) {
		$("#rightDiv .verticalTabBarClass tab-bar-button", parent.document).not("#pbs-tab").find(".verticalClass").addClass("covered");
		...
		$("#rightDiv .verticalTabBarClass tab-bar-button", parent.document).not("#pbs-tab").find(".verticalClass").removeClass("covered")
		adPanel0.style.display = 'none';
	}
	if (on) {
		$("#pbs-tab .verticalClass", PWD).css("background-color", "rgb(49, 96, 191)").css("color", "white")
	} else {
		$("#pbs-tab .verticalClass", PWD).css("background-color", "white").css("color", "black")
	}
}
...
function addBBOalertTab() {
	if (parent.document.getElementById('pbs-tab') != null) return;
	...
	t.querySelector('.verticalClass').textContent = 'PBS';
	t.id = 'pbs-tab';
	t.onclick = toggleOptions;
	t.style.color = 'white';
	t.style.display = "";
	vt.appendChild(t);
	t = parent.document.getElementById('pbs-tab');
	t.onclick = toggleOptions;
}
...
function setStatTextDiv() {
	if (parent.document.getElementById('pbs-statText') != null) return;
...
function setStatText(txt) {
	var st = parent.document.getElementById('pbs-statText');
	if (st == null) return;
	st.textContent = txt;
	if (txt != '') {
...
function setTabEvents() {
	...
	for (var i = 0; i < tabs.length; i++) {
		if (tabs[i].textContent.search('PBS') == -1) {
...
function hover_bboalert() {
	try {
		var t = window.parent.document.getElementById('pbs-tab');
...
function setBiddingButtonEvents() {
	// no-op: alert interception removed in PBS extension
}
